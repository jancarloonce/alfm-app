...
async function runBacktest(db) {
  ...
  const recordDates = ['2026-01-29', '2026-02-26', '2026-03-27', '2026-04-28']
  const results = []
  ...
  for (let i = 2; i < allDocs.length; i++) {
    ...
    // Count effective monthly buys from already-processed results (same month)
    ...
    const return1d = forward1 != null
      ? parseFloat(((forward1 - today.navpu) / today.navpu * 100).toFixed(4))
      : null
    const return3d = forward3 != null
      ? parseFloat(((forward3 - today.navpu) / today.navpu * 100).toFixed(4))
      : null
    const return5d = forward5 != null
      ? parseFloat(((forward5 - today.navpu) / today.navpu * 100).toFixed(4))
      : null
